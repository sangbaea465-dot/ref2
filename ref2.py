...
# 환경 변수 로드 제거: API Key는 사이드바에서 입력받아 사용
...
with st.sidebar:
    ...
    api_key_input = st.text_input("API Key를 입력하세요", type="password", value=st.session_state.api_key)
    if api_key_input != st.session_state.api_key:
        st.session_state.api_key = api_key_input
    has_api_key = bool(st.session_state.api_key)

    if not has_api_key:
        st.info("API Key를 입력하면 파일 처리 및 채팅이 활성화됩니다.")
    ...
    if uploaded_files:
        process_button = st.button("파일 처리하기", disabled=not has_api_key)
            ...
            with st.spinner("PDF 파일을 처리 중입니다..."):
                try:
                    ...
                    if not all_docs:
                        ...
                        # 임베딩 및 벡터 스토어 생성 (사이드바 API Key 사용)
...
# 사용자 입력 영역
if prompt := st.chat_input("질문을 입력하세요", disabled=not bool(st.session_state.api_key)):
    # 사용자 메시지 추가
        ...
        with st.spinner("답변을 생성 중입니다..."):
            try:
                if not st.session_state.api_key:
                    raise ValueError("API Key가 설정되지 않았습니다. 사이드바에 입력해주세요.")
                # RAG 검색 (상위 3개 문서만 사용)
